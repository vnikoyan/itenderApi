...
class OrganizeItenderTransformer extends AbstractTransformer
{
	/**
	 * User ID.
		...
		switch ($field) {
			...
			case 'participants':
				...
				foreach ($participants as $participant) {
					if($participant){
						if(count($participant->lots) && $participant->lots && $participant->lots[0]->userInfo && $participant->lots[0]->userInfo->id){
							$participant->suggestion = Suggestions::where('provider_id', $participant->lots[0]->userInfo->id)->where('organize_id', $item->id)->first();
						}
            ...
            case 'procurement':
                ...
                return $transformer;
			case 'purchase_count':
				return (int) $item->purchase_count;
			case 'one_time_transactions':
				return (int) $item->one_time_transactions;
			case 'itender_edited':
				return (boolean) $item->itender_edited;
