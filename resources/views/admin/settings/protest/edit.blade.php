...
@section('page_title') Բողոքարկում  @stop
...
<div class="row">
    <div class="col-lg-8 mx-auto">
        <div class="card dr-pro-pic">
            <div class="card-body">
                <div class="">
                    {{ Form::model($protest,array('route' => array('protest.update',$protest->id),'class'=>'form-horizontal form-material mb-0','files' => true))}}
                        @method('PUT')
                            ...
                            <div class="col-md-12">
                                <div class="form-group row">
                                    @foreach($language as $key => $value)
                                        <div class="col-md-3">
                                            {{ Form::text('title['.$value->code.']', $protest->getTranslation("title",$value->code), ["class"=>"form-control","placeholder"=>"Անվանում ".$value->name])}}
                                            <p style="color:red;height: 14px;width: 100%;"> {{ $errors->first('title.'.$value->code) }} </p>
                                        </div>  
                                        <div class="col-md-9">
                                            {!! Form::textarea('description['.$value->code.']', $protest->getTranslation("description",$value->code), ['class'=>'form-control html' ,"placeholder"=>"Նկարագրություն ".$value->name]) !!}
                        ...
                        <div class="form-group row">
                            <div class="col-md-3">
                                {{ Form::number('order', $protest->order, ["class"=>"form-control","placeholder"=>"order"])}}
                                <p style="color:red;height: 14px;width: 100%;"> {{ $errors->first('order') }} </p>
                            </div>  
                        </div>
                        <div class="form-group">
                            <a href="/admin/protest"  class="btn btn-gradient-danger btn-sm text-light px-4 mt-3 float-right mb-0 ml-2">Չեղարկել</a>
